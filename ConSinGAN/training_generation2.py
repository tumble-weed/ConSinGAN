...
def train(opt):
    ...
    fixed_noise = []
    noise_amp = []
    ds = [init_D(opt) for _ in range(opt.stop_scale+1)]
    for scale_num in range(opt.stop_scale+1):
        ...
        fixed_noise, noise_amp, generator, d_curr = train_single_scale(ds, generator, reals, fixed_noise, noise_amp, opt, scale_num, writer)
...
def train_single_scale(netDs, netG, reals, fixed_noise, noise_amp, opt, depth, writer):
    ...
    optimizerD = optim.Adam([p for netD in netDs for p in netD.parameters()], lr=opt.lr_d, betas=(opt.beta1, 0.999))
    ...
    for iter in _iter:
        ...
        for j in range(opt.Dsteps):
            ...
            for netD in netDs:
                netD.zero_grad()
            assert TODO, 'we"ll need real at all scales'
            output = netD(real)
            outputs = TODO
            errD_reals = []
            errD_real = 0
            for o in outputs:
                this_errD_real = -o.mean()
                errD_reals.append(this_errD_real)
                errD_real +=  this_errD_real 
